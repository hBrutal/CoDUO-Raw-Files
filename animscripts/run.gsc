...
main()
{
	...
	for (;;)
	{
		...
		shootWhileRunning =	(!animscripts\combat::NeedsToReload(runningReloadFraction)) &&
							( !isDefined(self.grenade) || !(GetDifficulty()=="easy" || GetDifficulty()=="medium") || self.team=="allies" ) &&
							...
							(self animscripts\utility::weaponAnims() != "lmg") &&
		...
		/#[[anim.locSpam]](" RA2 (shootWhileRunning is "+shootWhileRunning+")");#/

		// So if I'm allowed to shoot while running, do that.  Otherwise run normally.
		if (shootWhileRunning)
		{
			/#[[anim.println]]("Run script: shootWhileRunning, current pose: ",self.pose,", desired pose: ",desiredPose);#/
...
//			self thread MakeRunSounds ( "killSoundThread" );
		...
		{
			/#[[anim.println]]("Run script: not shootWhileRunning, current pose: ",self.anim_pose,", desired pose: ",desiredPose);#/
			self animscripts\face::SetIdleFace(anim.alertface);
			/#debugMovement = self.anim_movement;#/
			/#debugPose = self.anim_pose;#/
			/#[[anim.locSpam]](" RE1 ("+desiredPose+")");#/
			...
			case "stand":
				if ( self animscripts\utility::IsInCombat() )
				{
					...
					{
						self setanimknoball(%combatrun, %body, 0.1, 0.5, self.runanimplaybackrate);
					}
					else
					{
						self setanimknoball(%combatrun, %body, 1, 0.5, self.runanimplaybackrate);
					}
				...
				{    
					...
					else
					{
						...
						self setflaggedanimknoball("runanim", %combatrun, %body, 1, 0.2, self.runanimplaybackrate);
					}
					//self thread MakeRunSounds ( "killSoundThread" );
					self thread animscripts\shared::DoNoteTracksForever("runanim", "end runanim", undefined, timesThrough);
					wait 0.2;
			...
			case "crouch":
				...
				{
					self setflaggedanimknoball("runanim", self.crouchrun_combatanim, %body, 1, 0.4, self.runanimplaybackrate);
				}
				else
				{
					...
					self setflaggedanimknoball("runanim", %crouchrun, %body, 1, 0.2, self.runanimplaybackrate);
				}
				//self thread MakeRunSounds ( "killSoundThread" );
				self thread animscripts\shared::DoNoteTracksForever("runanim", "end runanim", undefined, timesThrough);
				wait 0.2;
