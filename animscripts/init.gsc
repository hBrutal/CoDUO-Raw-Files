...
main()
{
	...
	{
		println( "WARNING: Some AI has been give a blank weapon string.  It should be a valid weapon name or 'none'");
		self.weapon = "none";
	}
...
//	self.anim_lastDebugPrint1 = printString1;
//	self.anim_lastDebugPrint2 = printString2;
//	self.anim_lastDebugPrint3 = printString3;
//	self.anim_lastDebugPrint4 = printString4;
//	self.anim_lastDebugPrint5 = printString5;
//	self.anim_lastDebugPrint6 = printString6;
//	self.anim_lastDebugPrint7 = printString7;
//	self.anim_lastDebugPrint8 = printString8;
//	self.anim_lastDebugPrint9 = printString9;
	/#
...
AnimLocSpam(printString)
{
	...
	self.anim_lastLocSpam = printString;
	#/
}
...
SetupUniqueAnims()
{
	...
	else if ( (self animscripts\utility::weaponAnims() == "lmg") )
	{
		self.anim_combatrunanim = %c_mg_mg34_run;
		self.anim_crouchrunanim = %crouchrun_loop_forward_1;
		self.run_noncombatanim = %c_mg_mg34_run;
//		self.walk_combatanim = %c_mg_mg34_run;	
//		self.walk_noncombatanim = %c_mg_mg34_run;	
	}
	else
	{
	...
	if (!isDefined(self.runanimplaybackrate))
	{
		// allies need to run a bit faster
		if ( !isDefined( self.team ) || self.team == "axis" )
		{
			self.runanimplaybackrate = 0.8 + randomfloat(0.4);
		}
		else
		{
			self.runanimplaybackrate = 0.9 + randomfloat(0.4);
		}
	}
}
