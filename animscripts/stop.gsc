...
main()
{
	...
	for(;;)
	{
		...
		if (usingLMG)
		{
			...
			// if we are stopped but not at the goal then play the appropriate idle anim
			/#[[anim.locSpam]]("s3e");#/
...
CoverLeftStandStill(cornerAngle, changeReason)
{
		...
		)
	{
		...
		{
			/#[[anim.println]]("Angle difference is ",angleDifference,", chose set a");#/
			idleSet = "a";
		}
		else
		{
			/#[[anim.println]]("Angle difference is ",angleDifference,", chose set b");#/
			idleSet = "b";
		}
	}
	else
...
CoverLeftStandStill_a(cornerAngle)
{
	...
	if ( self animscripts\utility::IsInCombat() )
	{
		/#[[anim.println]]("CoverLeftStandStill_a (in combat)");#/
	...
	{
		/#[[anim.println]]("CoverLeftStandStill_a (not in combat)");#/
...
CoverLeftStandStill_b(cornerAngle)
{
	...
	if ( self animscripts\utility::IsInCombat() )
	{
		/#[[anim.println]]("CoverLeftStandStill_b (in combat)");#/
		self.anim_alertness = "alert";
	...
	{
		/#[[anim.println]]("CoverLeftStandStill_b (not in combat)");#/
		self.anim_alertness = "casual";
