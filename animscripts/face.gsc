...
{
	// To prevent certain AI to say any generic dialogue
	if(isdefined(self.generic_dialogue) && !self.generic_dialogue)
	{
		return;
	}
...
PlayFaceThread(facialanim, soundAlias, importance, notifyString, waitOrNot, timeToWait)
{
	...
	if ( !isDefined(facialanim) && !isDefined(soundAlias) )
	{
		...
		/#[[anim.assert]](0, "Either facialanim or soundAlias should be defined when calling PlayFaceThread or SaySpecificDialogue");#/
